...
def copy_repo_to_bucket(bucket, cluster_name):
    """Copies the repo to the bucket and returns the setup.py path."""
    this_repo = os.path.dirname(os.path.dirname(os.getcwd()))
    copy_location = os.path.join("gs://%s" % bucket, cluster_name)
    cmd = "gsutil cp -r %s/* %s" % (this_repo, copy_location)
    os.system(cmd)
    return os.path.join(copy_location, 'install.sh')
...
def create_cluster(dataproc, project, zone, region, bucket_name, cluster_name):

    # Copy the repo
    init_script = copy_repo_to_bucket(bucket_name, cluster_name)
...
def main(project_id, zone, cluster_name, bucket_name,
    ...
    if True:
        ...
        if create_new_cluster:
            create_cluster(
                dataproc, project_id, zone, region, bucket_name, cluster_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--zone', help='Zone to create clusters in/connect to')
    parser.add_argument(
        '--cluster_name',
        help='Name of the cluster to create/connect to')
    parser.add_argument(
        '--gcs_bucket', help='Bucket to upload Pyspark file to')
    parser.add_argument(
